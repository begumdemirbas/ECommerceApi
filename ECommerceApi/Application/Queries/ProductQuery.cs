...
namespace ECommerceApi.Application.Queries
{
    public class ProductQuery: IProductQuery
    {
        ...
        public async Task<ProductDataModel> GetByIdAsync(KeyInputModel inputModel)
        {
            var model = new ProductDataModel();
            var product = await _productRepository.GetByKeyAsync(inputModel.Id);
            if (product == null)
        ...
        public async Task<ProductListDataModel> GetAllAsync()
        {
            ...
            else
            {
                ...
                model.Data = (await _productRepository.GetAllAsync()).Select(product => new ProductDataModel
                {
                    Id = product.Id,
                    Name = product.Name,
                    Description = product.Description,
